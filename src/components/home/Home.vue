<template>
	<div>
		<h2 class="preserve-whitespace">{{ $t("homepage.welcome") }}</h2>
		<p class="page-summary">
			{{ $t("homepage.siteDescription") }}
		</p>

		<div class="box">
			<v-row>
				<v-col cols="12" md="6">
					<v-btn class="btn btn-primary" @click="goToRoute('/utvekslinger')">{{ $t("operations.viewExchanges")
					}}</v-btn>
				</v-col>
				<v-col cols="12" md="6">
					<v-btn class="btn btn-third" @click="goToRoute('/min_utveksling')">{{ $t("operations.createExchange")
					}}</v-btn>
				</v-col>
			</v-row>
		</div>

		<!-- <p class="box box-success preserve-whitespace text-center">
			{{ $t("homepage.newFeature") }}
		</p> -->


		<div class="box box-countries">
			<WorldMap :countries="countriesToHighlight" />
		</div>

		<br />
		<v-divider />
		<br />

		<div class="top-countries-and-top-study-programs">
			<h3>{{ $t("homepage.topCountries") }}</h3>
			<BarChart :items="topCountriesTranslated" />
		</div>

		<div class="top-countries-and-top-study-programs">
			<h3>{{ $t("homepage.topStudyPrograms") }}</h3>
			<BarChart :items="topStudyPrograms" />
		</div>


	</div>
</template>

<script>
import WorldMap from "./WorldMap.vue";
import BarChart from "./BarChart.vue";
import countriesInformation from "../../data/countriesInformation.json";
import { db, auth } from "../../js/firebaseConfig.js";
import { getFlagUrl } from '../../js/i18nHelpers';

import { getExchangesData } from "../../js/exchangesCache";

export default {
	name: "App",
	components: {
		WorldMap,
		BarChart,
	},
	setup() {
		const user = auth.currentUser;
		return { user };
	},
	data() {
		return {
			countriesToHighlight: [],
			topCountries: [],
			topStudyPrograms: [],
			countriesInfo: countriesInformation,
		};
	},
	mounted() {
		this.getValuesFromDatabase();
	},
	computed: {
		topCountriesTranslated() {
			return this.topCountries.map((country) => {
				return {
					name: this.$t(`countries.${country.name}`),
					count: country.count,
				};
			});
		}
	},
	methods: {
		async getValuesFromDatabase() {
			try {
				const exchanges = await getExchangesData();

				if (!exchanges || Object.keys(exchanges).length === 0) {
					console.error("No data available");
					return;
				}

				const countriesSet = new Set();
				const countriesCount = {};
				const studyProgramsCount = {};

				for (const exchangeKey in exchanges) {
					const exchange = exchanges[exchangeKey];

					const autumnCourses = Array.isArray(exchange.courses?.Høst)
						? exchange.courses.Høst
						: Object.values(exchange.courses?.Høst || {});

					const springCourses = Array.isArray(exchange.courses?.Vår)
						? exchange.courses.Vår
						: Object.values(exchange.courses?.Vår || {});

					const summerCourses = Array.isArray(exchange.courses?.Sommer)
						? exchange.courses.Sommer
						: Object.values(exchange.courses?.Sommer || {});

					const hasAutumnCourses = autumnCourses.length > 0;
					const hasSpringCourses = springCourses.length > 0;
					const hasSummerCourses = summerCourses.length > 0;

					const hasMultipleSemesters =
						[hasAutumnCourses, hasSpringCourses, hasSummerCourses].filter(Boolean).length >= 2;

					const hasAnyCourses =
						hasAutumnCourses || hasSpringCourses || hasSummerCourses;

					if (hasMultipleSemesters) {
						if (!exchange.sameUniversity && exchange.secondCountry && exchange.secondCountry !== "null") {
							countriesSet.add(exchange.secondCountry);
							countriesCount[exchange.secondCountry] = (countriesCount[exchange.secondCountry] || 0) + 1;
						}

						if (exchange.country) {
							countriesSet.add(exchange.country);
							countriesCount[exchange.country] = (countriesCount[exchange.country] || 0) + 1;
						}
					} else if (hasAnyCourses) {
						if (exchange.country) {
							countriesSet.add(exchange.country);
							countriesCount[exchange.country] = (countriesCount[exchange.country] || 0) + 1;
						}
					}

					if (exchange.study) {
						studyProgramsCount[exchange.study] = (studyProgramsCount[exchange.study] || 0) + 1;
					}
				}

				this.topCountries = Object.entries(countriesCount)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 10)
					.map(([name, count]) => ({ name, count }));

				this.topStudyPrograms = Object.entries(studyProgramsCount)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 10)
					.map(([name, count]) => ({ name, count }));

				this.countriesToHighlight = Array.from(countriesSet).map((countryName) => ({
					name: countryName,
				}));
			} catch (error) {
				console.error("Error fetching values from database:", error);
			}
		},
		goToRoute(route) {
			this.$router.push(route);
		},
		getFlagUrl
	},
};
</script>

<style scoped>
/* Box Styles */
.box {
	border-radius: 10px;
	padding: 15px;
	margin: 15px auto;
	font-size: 16px;
	width: fit-content;
	max-width: 90%;
}

.box-countries {
	width: 100%;
	height: 100%;
	/* Ensure this has height */
}

.top-countries-and-top-study-programs {
	display: flex;
	flex-direction: column;
	align-items: center;
	text-align: center;

	width: 100%;
	max-width: 1000px;
	margin: 25px auto;
	/* mindre luft */
	padding: 10px 0 20px 0;
	/* minimal padding */

}


@media screen and (max-width: 600px) {
	.btn {
		width: 100% !important;
	}

	.top-countries-and-top-study-programs {
		margin: 10px auto;
		padding: 0;
		position: relative;
		z-index: 10;
		pointer-events: auto;
	}


	.top-countries-and-top-study-programs h3 {
		font-size: 1.3rem;
	}
}
</style>
