# Terms of Use

## 1. Purpose

Utvekslingsportalen provides informational content based on user-submitted exchange experiences. It is intended as a support tool and is **not an authoritative source**.

## 2. Not an official NTNU service

Utvekslingsportalen is a student-driven, non-commercial project and is **not affiliated with or endorsed by NTNU**.

## 3. User responsibilities

By using the platform, you agree:

- Not to submit sensitive personal data or private information about yourself or others
- To submit content that is accurate to the best of your knowledge
- To understand that exchange rules and course equivalencies may change

## 4. Accounts

You are responsible for:

- Keeping access to your account secure
- Using the platform only for its intended purpose

## 5. Content moderation

The project owner may edit, hide, or remove content that is:

- Clearly incorrect or misleading
- Inappropriate, abusive, or harmful
- Unrelated to the platform’s purpose

## 6. No guarantees

Information may be outdated or incomplete.  
Users must verify course approvals and exchange requirements with your home university and partner institutions.

## 7. Limitation of liability

The platform is provided “as is”. The project owner is not responsible for outcomes or decisions made based on information on the site.

## 8. Changes

These terms may be updated as the platform evolves. Continued use means acceptance of updated terms.
